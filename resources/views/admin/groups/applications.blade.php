...
@section('title', 'グループメンバー・申請管理')

@section('content')
<div class="px-4 py-2">
    <div class="flex items-center justify-between mb-6">
        <h2 class="text-2xl font-bold text-gray-800">グループメンバー・申請管理<x-help help-key="admin.groups.applications" /></h2>
        <a href="{{ route('admin.groups.create') }}" class="bg-admin text-white px-4 py-2 rounded-lg hover:bg-admin-dark transition font-bold text-sm">
            + 新規グループ作成
        </a>
    </div>
    ...
    @forelse($groups as $group)
        ...
        <div class="bg-gray-50 px-6 py-3 border-b border-gray-100 flex justify-between items-center">
            <div class="flex items-center space-x-2">
                <h3 class="font-bold text-gray-700">{{ $group->name }}</h3>
                <span class="text-sm text-gray-500">（{{ $group->description ?? '説明はありません。' }}）</span>
            </div>

            {{-- ★追加：アクションボタン（編集・削除） --}}
            <div class="flex items-center space-x-2">
                {{-- 編集ボタン --}}
                <a href="{{ route('admin.groups.edit', $group->id) }}" class="text-gray-400 hover:text-blue-600 transition p-1" title="グループを編集">
                    <span class="material-symbols-outlined text-[20px]">edit</span>
                </a>

                {{-- 削除ボタン --}}
                <form action="{{ route('admin.groups.destroy', $group->id) }}" method="POST" 
                    onsubmit="return confirm('グループ「{{ $group->name }}」を完全に削除しますか？\n※このグループが必要なイベントの制限も解除されます。')">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="text-gray-400 hover:text-red-600 transition p-1" title="グループを削除">
            ...
            <div class="p-6">
                ...
                <div class="mb-8">
                    <h4 class="text-sm font-bold text-orange-500 uppercase tracking-wider mb-4 flex items-center">
                        ...
                        未承認の申請<x-help help-key="admin.groups.unverified" />
                    </h4>
                    @php $pendingUsers = $group->users->where('pivot.status', 'pending'); @endphp
                ...
                <div>
                    <h4 class="text-sm font-bold text-gray-500 uppercase tracking-wider mb-4 flex items-center">
                        ...
                        承認済みメンバー<x-help help-key="admin.groups.authenticated" />
                    </h4>
                    @php $approvedUsers = $group->users->where('pivot.status', 'approved'); @endphp
    ...
    @empty
        <div class="bg-white p-12 rounded-xl shadow-sm border border-gray-100 text-center">
            <p class="text-gray-500">作成済みのグループがありません。</p>
        </div>
    @endforelse
</div>
@endsection