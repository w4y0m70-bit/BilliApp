<div x-data="{ openModal: null }" class="p-2 flex flex-wrap -mx-2">
    @foreach ($events as $event)
        <div class="w-full sm:w-1/2 md:w-1/3 lg:w-1/4 px-2 mb-2">
            <!-- カード -->
            <div class="bg-white shadow rounded-lg p-4 border cursor-pointer" @click="openModal = {{ $event->id }}">
                <div class="flex justify-between items-start mb-1">
                    <div class="flex items-center gap-2">
                        <h4 class="text-lg font-semibold text-admin">{{ $event->title }}</h4>
                        <x-event.team-size-icon :size="$event->max_team_size" :show-label="false" compact="true" />
                    </div>
                    <div class="flex justify-between items-start">
                        ...
                        {{-- ① 過去イベントは最優先で複製アイコン --}}
                        @if ($isPast)
                            <a href="{{ route('admin.events.replicate', $event) }}" @click.stop
                                class="text-gray-700 hover:text-blue-700" title="このイベントを元に新規作成">
                                <span class="material-symbols-outlined">content_copy</span>
                            </a>

                            {{-- ② それ以外 → 編集アイコン --}}
                        @else
                            <a href="{{ route('admin.events.edit', $event) }}" @click.stop
                ...
                @if ($event->requiredGroups->isNotEmpty())
                    <div class="flex flex-wrap gap-1 mb-2">
                        @foreach ($event->requiredGroups as $group)
                            <span
                                class="inline-flex items-center text-xs px-1.5 py-0.5 rounded bg-blue-50 text-blue-700 border border-blue-100 font-bold">
                                {{ $group->name }}
                            </span>
                        @endforeach
                    </div>
                ...
                <div class="mb-2">
                    <div class="flex flex-wrap gap-1 items-center">
                        <span class="text-sm font-bold text-gray-500 mr-1 whitespace-nowrap">クラス:</span>
                        @forelse($event->eventClasses as $class)
                            @php
                                $classEnum = \App\Enums\PlayerClass::tryFrom($class->class_name);
                            @endphp
                            @if ($classEnum)
                                <x-event.class-tag size="md" :bgColor="$classEnum->color()">
                                    {{ $classEnum->shortLabel() }}
                                </x-event.class-tag>
                            @else
                                <span
                                    class="text-[10px] px-1 border rounded bg-gray-100">{{ $class->class_name }}</span>
                            @endif
                        @empty
                            <span class="text-red-500 text-[10px]">ー</span>
                        @endforelse
                    </div>
                </div>
                ...
                <div class="text-sm mt-2 p-2 bg-gray-50 rounded border border-gray-100">
                    <x-event.entry-status :event="$event" :href="route('admin.events.participants.index', $event->id)" />
                </div>

            </div>

            <!-- モーダル -->
            <div x-show="openModal === {{ $event->id }}" x-transition
                ...
                <div class="bg-white rounded-lg shadow-lg max-w-lg w-full p-6 relative max-h-[80vh] overflow-y-auto">
                    <button class="absolute top-2 right-2 text-gray-600 text-2xl"
                        @click="openModal = null">&times;</button>

                    <h3 class="text-xl font-semibold mb-4 border-b pb-2">{{ $event->title }}</h3>

                    <div class="grid grid-cols-1 gap-2 mb-4 text-sm">
                        <p><span
                                class="font-bold w-20 inline-block">開催日：</span>{{ $event->event_date->isoFormat('YYYY/MM/DD（ddd）HH:mm') }}
                        </p>
                        <p><span
                                class="font-bold w-20 inline-block">締　切：</span>{{ $event->entry_deadline->isoFormat('YYYY/MM/DD（ddd）HH:mm') }}
                        </p>
                        <p><span
                                class="font-bold w-20 inline-block">公開日：</span>{{ $event->published_at ? $event->published_at->isoFormat('YYYY/MM/DD（ddd）HH:mm') : '未設定' }}
                        </p>
                    </div>

                    <div class="bg-gray-50 p-3 rounded mb-4 text-sm">
                        <p class="font-bold mb-2 text-gray-800 text-xs">【募集クラス詳細】</p>
                        <div class="flex flex-wrap gap-1.5">
                            @forelse($event->eventClasses as $class)
                                ...
                                @if ($classEnum)
                                    <x-event.class-tag size="md" :bgColor="$classEnum->color()">
                                        {{ $classEnum->shortLabel() }} {{-- モーダル内は略称ではなくフルネームでもOK --}}
                                    </x-event.class-tag>
                                @else
                                    <span
                                        class="bg-white border px-2 py-1 rounded shadow-sm text-xs">{{ $class->class_name }}</span>
                                @endif
                            @empty
                                <span class="text-red-500 text-xs">クラス設定なし</span>
                            @endforelse
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 gap-2 mb-4 text-sm">
                        <div class="flex items-center">
                            {{-- カード内と同じデザインで統一 --}}
                            <div class="flex items-center">
                                <span class="font-bold w-20 inline-block text-gray-700">形式：</span>
                                <x-event.team-size-icon :size="$event->max_team_size" />
                            </div>
                        </div>
                    ...
                    @if ($event->instruction_label)
                        <div class="mb-4 text-sm">
                            ...
                            <p class="p-2 bg-yellow-50 rounded border border-yellow-100">
                                {{ $event->instruction_label }}</p>
                        </div>
                    @endif
                    ...
                    <div class="mt-6 text-right">
                        <button class="px-6 py-2 bg-gray-600 text-white rounded hover:bg-gray-700"
