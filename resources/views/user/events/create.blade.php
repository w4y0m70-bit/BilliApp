...
@section('content')
    <div class="bg-white shadow rounded-lg p-6 max-w-lg mx-auto" x-data="{
        hasConflict: {{ $conflictingEntries->isNotEmpty() ? 'true' : 'false' }},
        {{-- 競合するイベント名と主催者名をセットで取得 --}}
        conflictEventDetails: '{{ $conflictingEntries->map(fn($e) => '【' . ($e->event->organizer->name ?? '主催者不明') . '】 ' . ($e->event->title ?? '無題'))->implode("\\n") }}',
    
        submitForm(e) {
            if (this.hasConflict) {
                {{-- 改行を含めたメッセージを作成 --}}
                const message = `同日（{{ $event->event_date->format('m/d') }}）に以下のイベントへ既にエントリー済みですが、よろしいですか？\n\n${this.conflictEventDetails}`;
        ...
        <form action="{{ route('user.entries.entry', $event->id) }}" method="POST" class="space-y-6"
            ...
            @php
                ...
                if ($userClass && $event->eventClasses->isNotEmpty()) {
                    ...
                    $bestMatch = $event->eventClasses
                        ->map(function ($item) use ($userRank) {
            ...
            <div>
                ...
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    @foreach ($event->eventClasses as $class)
                        ...
                        <label
                            class="flex items-center gap-2 p-3 border rounded cursor-pointer hover:bg-gray-50 has-[:checked]:border-user has-[:checked]:bg-user/5">
            ...
            @if ($event->instruction_label)
                <div>
                    ...
                    <textarea name="user_answer" rows="3" class="w-full border rounded p-2 focus:ring-user focus:border-user"
            ...
            @if ($event->entry_count >= $event->max_entries)
                <div
                    class="bg-orange-50 p-4 rounded-lg text-sm text-orange-800 border border-orange-200 flex items-start gap-3">
                    <span class="material-icons text-orange-500">info</span>
                    <div>
                        <p class="font-bold">現在キャンセル待ちでの受付となります</p>
                        <p class="mt-1 text-xs opacity-90">
                            エントリー枠（{{ $event->max_entries }}
                            {{ $event->max_team_size > 1 ? 'チーム' : '名' }}）が満員のため、欠員が出た場合のみ繰り上げとなります。
                        </p>
                    </div>
                </div>
            @endif
