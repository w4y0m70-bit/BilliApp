...
@section('content')
        ...
        @if (isset($invitations) && $invitations->isNotEmpty())
            <div class="mb-8 space-y-3">
                @foreach ($invitations as $invitation)
                    <div
                        class="bg-gradient-to-r from-user to-user-dark text-white p-4 rounded-xl shadow-lg flex flex-col sm:flex-row items-center justify-between gap-4 animate-pulse-subtle">
                        ...
                        <div class="flex gap-2 w-full sm:w-auto">
                            ...
                            <a href="{{ route('user.events.show', $invitation->event_id) }}"
                                class="flex-1 sm:flex-none bg-white text-user px-6 py-2 rounded-full text-sm font-bold hover:bg-gray-100 transition text-center shadow-md">
                                詳細を確認する
                            </a>
                        </div>
                    </div>
    ...
    @endif
        ...
        <div id="filter-overlay" onclick="toggleFilter()" class="fixed inset-0 bg-black/50 z-40 hidden transition-opacity">
        </div>

        <div id="filter-drawer"
            class="fixed top-0 right-0 h-full w-80 sm:w-96 bg-white z-50 shadow-2xl transform translate-x-full transition-transform duration-300 ease-in-out flex flex-col">
                        ...
                        @foreach ($groupedLocations as $pref => $cities)
                            <div class="border rounded-lg overflow-hidden border-gray-200">
                                <div class="flex items-center justify-between bg-gray-50 px-3 py-2">
                                    ...
                                    <button type="button" onclick="toggleCityList('{{ $pref }}')"
                                        class="p-1 hover:bg-gray-200 rounded-full">
                                        <span id="icon-{{ $pref }}"
                                            class="material-symbols-outlined text-gray-400 block transition-transform">expand_more</span>
                                    </button>
                                </div>

                                <div id="list-{{ $pref }}"
                                    class="hidden p-3 bg-white grid grid-cols-2 gap-2 border-t border-gray-100">
                                    @foreach ($cities as $city)
                                        <label class="flex items-center space-x-2 cursor-pointer group">
                                            <input type="checkbox" name="cities[]" value="{{ $city }}"
                                                ...
                                                class="rounded border-gray-300 text-user focus:ring-user">
                                            <span
                                                class="text-xs text-gray-500 group-hover:text-user">{{ $city }}</span>
                                        </label>
                                    @endforeach
                                </div>
                            </div>
            ...
            <div class="p-4 border-t bg-white flex flex-col space-y-2">
                <button type="submit" form="filter-form"
                    class="w-full bg-user text-white py-3 rounded-full font-bold shadow-lg text-sm">
                    この条件で絞り込む
                </button>
                <a href="{{ route('user.events.index') }}"
                    class="text-center text-xs text-gray-400 hover:text-gray-600 underline py-1">
                    条件をリセット
                </a>
            </div>
        </div>
                    ...
                    <div class="block bg-white shadow rounded-xl p-4 border hover:shadow-lg transition">
                        ...
                        <div x-data="{ showOrganizer: false }" class="mb-1">
                            <div class="flex justify-between items-start gap-2">
                                <button @click="showOrganizer = !showOrganizer" type="button"
                                ...
                                @if ($event->requiredGroups->isNotEmpty())
                                    <div class="flex flex-wrap gap-1 justify-end">
                                        @foreach ($event->requiredGroups as $group)
                                            <span
                                                class="inline-flex items-center text-[10px] px-2 py-0.5 rounded-full bg-blue-100 text-blue-700 font-bold border border-blue-200">
                                                {{ $group->name }}限定
                                            </span>
                                        @endforeach
                                    </div>
                            ...
                            {{-- ★ 主催者連絡先の簡易パネル --}}
                            @if ($event->organizer)
                                <div x-show="showOrganizer" x-transition
                                    class="mt-2 p-3 bg-gray-50 border border-gray-100 rounded text-[11px] text-gray-600 space-y-2 shadow-inner">
                                    <div class="flex flex-col space-y-0.5">
                                        <div class="flex justify-between items-center">
                                            <span
                                                class="font-semibold text-gray-500 underline underline-offset-2 decoration-gray-300">所在地</span>
                                            {{-- マップを開くアイコンリンク --}}
                                    ...
                                    <div class="grid grid-cols-2 gap-2 border-t border-gray-200 pt-2">
                                        ...
                                        <div class="flex flex-col overflow-hidden">
                                            ...
                                            <a href="mailto:{{ $event->organizer->email }}"
                                                class="text-sm text-blue-600 hover:underline truncate font-medium">
                        ...
                        <h3 class="text-2xl font-black mb-1 text-user">
                            ...
                            @if ($event->max_team_size > 1)
                                <span
                                    class="inline-block ml-2 px-2 py-0.5 text-xs font-bold text-white bg-user rounded-full">
                        ...
                        <div class="mt-2 flex items-start gap-1">
                            ...
                            <div class="flex flex-wrap gap-1">
                                ...
                                @forelse($eventClasses as $eventClass)
                                    ...
                                    @endphp
                                    <span
                                        class="inline-flex items-center text-[12px] px-1.5 py-0.2 rounded bg-gray-100 text-gray-600 border border-gray-300 font-medium">
                        ...
                        <div class="flex items-center mb-1">
                            <p class="text-sm text-gray-700 mt-1">
                                <strong>参加状況：</strong>
                                <x-event.entry-status :event="$event" :href="route('user.events.participants', $event->id)" />
                            </p>
                        ...
                        <div class="mt-3">
                            @if ($isInvited)
                                {{-- 相手から誘われている場合 --}}
                                <span
                                    class="inline-block bg-blue-600 text-white text-sm px-3 py-1 rounded animate-bounce-short shadow-md font-bold">
                                    招待が届いています
                                </span>
                            @elseif ($userEntry && $userEntry->members()->where('invite_status', 'pending')->exists())
                                ...
                                <div class="space-y-1">
                                    <span
                                        class="inline-block bg-yellow-500 text-white text-sm px-3 py-1 rounded font-bold shadow-sm">
                                        パートナーの回答待ち
                                    </span>
                                    @if ($status === 'waitlist')
                            ...
                            @elseif ($status === 'entry')
                                <span
                                    class="inline-block bg-user text-white text-sm px-3 py-1 rounded transition font-bold shadow-sm">
                                    エントリー中
                                </span>
                            @elseif ($status === 'waitlist')
                                <span
                                    class="inline-block bg-orange-500 text-white text-sm px-3 py-1 rounded transition font-bold shadow-sm">
                                    キャンセル待ち（{{ $userEntry->waitlist_position ?? '' }}番目）
                                </span>
                            @elseif ($isDeadlinePast)
                                <span
                                    class="inline-block bg-gray-500 text-white text-sm px-3 py-1 rounded transition text-xs">
                                    エントリー締切
                                </span>
                            @else
                                <span
                                    class="inline-block bg-gray-400 text-white text-sm px-3 py-1 rounded transition text-xs font-bold shadow-sm">
