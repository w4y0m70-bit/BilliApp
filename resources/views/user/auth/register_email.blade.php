    ...
    <div class="min-h-screen flex flex-col items-center justify-center bg-gray-100">
        <div class="bg-white p-8 rounded-xl shadow-md w-full max-w-md">
            ...
            <form action="{{ route('user.register.email.post') }}" method="POST">
                ...
                <button type="submit" class="w-full bg-user text-white font-bold py-2 rounded hover:bg-blue-700 transition">
                    登録用URLを送信する
                </button>
            </form>
        </div>
        <div class="mt-4 text-center">
            <a href="{{ route('user.login') }}" class="text-sm text-gray-600 hover:underline">
                ログイン画面へ戻る
            </a>
